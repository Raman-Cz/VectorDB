...
### Basic CPU IVF: Chosen Design, Not Implemented

The first IVF index is intentionally a single-threaded CPU, build-then-query implementation. It will be separate from product quantization, persistence, and live mutation support.
...
- Train centroids with full-batch spherical k-means using seed 42, `nlist = 316`, mean centroid movement below `1e-4`, and a 100-iteration safety cap.
...
The exact baseline takes approximately 5.7 ms per query at 100,000 vectors and 128 dimensions (175.63 QPS). If an IVF configuration probes about 10% of balanced inverted lists, it should scan roughly 10% of the vectors after its centroid-routing step. The initial expectation is a 5-10x QPS improvement with measurable recall loss. This is a prediction to test against recorded IVF results, not a performance claim.
